...
- B-024 exact canonical-label tracker exclusivity observation.
...
- B-024 is runtime observed at the external-dependency layer: an isolated fixture created the frozen canonical labels and showed GitHub’s additive label operations retain the prior `bug` + `needs-triage` roles when `enhancement` + `ready-for-agent` are added. This proves tracker non-exclusivity, not triage-agent prompt adherence.
